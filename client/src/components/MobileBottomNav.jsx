      ...
      >
        <div className="speed-dial-wrapper">
          ...
          <div className="subs">
            {/* Apply Leave */}
            {user?.role !== 'CEO' && (
              <button type="button" className="sub-circle bg-blue-600 hover:bg-blue-700" title="Apply Leave"
                onClick={(e) => { e.stopPropagation(); handleOpenApplyModal(); setTimeout(() => setIsSpeedDialOpen(false), 50); }}>
            ...
            {/* Attendance & Punch */}
            {user?.role !== 'CEO' && (
              <button type="button" className="sub-circle bg-teal-600 hover:bg-teal-700" title="Attendance & Punch"
                onClick={(e) => { e.stopPropagation(); goTo('/attendance'); }}>
                <Clock className="w-4 h-4 text-white" />
              </button>
            )}

            {/* Daily Work Reports */}
            <button type="button" className="sub-circle bg-violet-600 hover:bg-violet-700" title="Daily Work Reports"
              onClick={(e) => { e.stopPropagation(); goTo('/daily-reports'); }}>
              <FileText className="w-4 h-4 text-white" />
            </button>

            {/* Notifications */}
            <button type="button" className="sub-circle bg-rose-500 hover:bg-rose-600" title="Notifications"
              onClick={(e) => { e.stopPropagation(); goTo('/notifications'); }}>
            ...
            {/* Scroll to Top */}
            <button type="button" className="sub-circle bg-amber-500 hover:bg-amber-600" title="Scroll to Top"
              onClick={(e) => { e.stopPropagation(); scrollToTop(); setTimeout(() => setIsSpeedDialOpen(false), 50); }}>
              <ArrowUp className="w-4 h-4 text-white" />
            </button>

            {/* Holidays */}
            <button type="button" className="sub-circle bg-emerald-500 hover:bg-emerald-600" title="Holidays"
              onClick={() => goTo('/holidays')}>
              <Calendar className="w-4 h-4 text-white" />
            </button>

            {/* Reports */}
            <button type="button" className="sub-circle bg-indigo-600 hover:bg-indigo-700" title="Reports"

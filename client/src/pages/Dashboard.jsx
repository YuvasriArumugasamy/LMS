      ...
      {(user?.role === 'CEO' || user?.role === 'SUPER_ADMIN' || user?.role === 'HR' || user?.role === 'MANAGER') && (
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            <MetricCard title="TOTAL EMPLOYEES" value={stats?.cards?.totalEmployees || 128} icon={Users} color="primary" trend="+8.2% This Month" bgImage={blueBgCard} />
            <MetricCard title="DEPARTMENTS" value={stats?.cards?.totalDepartments || 12} icon={Building2} color="secondary" trend="+2 New This Month" bgImage={purpleBgCard} />
            <MetricCard title="PENDING APPROVALS" value={stats?.cards?.pendingLeaves || 7} icon={Clock} color="warning" trend="+2 Since Yesterday" bgImage={orangeBgCard} />
            <MetricCard title="EMPLOYEES ON LEAVE" value={stats?.cards?.employeesOnLeaveToday || 18} icon={CalendarCheck} color="success" trend="+5.4% This Month" bgImage={greenBgCard} />
          </div>
        </div>
      )}
